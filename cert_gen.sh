...
# /CN=localhost is for Common Name or domain name
# /emailAddress=leolegrand1014@gmail.com is for email address

openssl req -x509 -newkey rsa:4096 -days 365 -nodes -keyout ./cert/ca-key.key -out ./cert/ca-cert.crt -subj "/C=/ST=/L=/O=/OU=/CN=localhost/emailAddress="

echo "CA's self-signed certificate"
openssl x509 -in ./cert/ca-cert.crt -noout -text

# 2. Generate web server's private key and certificate signing request (CSR)
openssl req -newkey rsa:4096 -nodes -keyout ./cert/server-key.key -out ./cert/server-req.csr -subj "/C=/ST=/L=/O=/OU=/CN=localhost/emailAddress="
...
openssl req -newkey rsa:4096 -nodes -keyout ./cert/client-key.key -out ./cert/client-req.csr -subj "/C=/ST=/L=/O=/OU=/CN=localhost/emailAddress="
